  ...
  private
    ...
    def reconcile_search_index
      ...
      missing_from_search.each do |missing_id|
        Rails.logger.info("Re-enqueuing completed medium #{missing_id} missing from MampfSearch index.")
        MampfsearchIngestJob.perform_later(missing_id)
      end
    rescue SearchClient::MampfSearchError => e
      Rails.logger.warn("Search index reconciliation skipped: #{e.message}")
    end
end
